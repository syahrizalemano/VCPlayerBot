    ...
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        if Config.ADMIN_ONLY:
            if not (message.from_user is None and message.sender_chat or message.from_user.id in admins):
                k=await message.reply_sticker("CAACAgUAAxkBAAL1tWIZ5VLPT66Ehfo6VQfEaRHIA014AAIMBQACPFPwV4M6LFvB4RU-IwQ")
...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAACAgUAAxkBAAL1uWIZ5a9BUEeMqhrERx2qx6WSPvRSAAJ8AwAC1TRwVmDNbtOr8nvUIwQ", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️Join Here', url='https://t.me/idnswascome')]]))
    await delete_messages([m, k])
    ...
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        ...
        buttons = [
            [
                ...
                InlineKeyboardButton('🧩 Join Here', url='https://t.me/idnswascome'),
            ]
            ]
